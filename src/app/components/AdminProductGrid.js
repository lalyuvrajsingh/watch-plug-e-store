  ...
  return (
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4'>
            {products.map(product => (
              <div className="border h-fit  flex flex-col items-center justify-center hover:shadow-2xl transition-shadow ease-in-out rounded-2xl m-1 my-3 shadow-xl p-4" key={product.id}>
              {/* <p className=" mb-5 text-gray-400">Added on: {product.date ? `${product.date}` : 'Date unknown'}</p> */}
                <img src={product.coverImage} alt={product.name} className="h-40 my-3 object-cover" />
                  ...
                  <button onClick={() => handleDelete(product.id)} className="bt border m-2 p-1 rounded-xl bg-black text-white shadow-lg">Delete</button>
                  </div>
              </div>
            ))}
          </div> 
